...
capture program drop DefineQuadraticConstraints

program DefineQuadraticConstraints

	args 	MyOutcomeVariable

	* First, figure out how many controls there are...

	quietly 	reg `MyOutcomeVariable' SQ_fort_*
	global 		ControlCount 		= e(rank)

	* Second, figure out the total number of variables we will need...

	quietly 	reg `MyOutcomeVariable' SQ_tg_*_* SQ_fort_*
	...
	* Third, count each of the separate treatment dummy sets...
	...
	forvalues TreatmentLabel = 1/2 ///
		...
		if `TreatmentLabel' != 5 ///
		{
		quietly reg `MyOutcomeVariable' SQ_tg_`TreatmentLabel'_*_* SQ_fort_*
