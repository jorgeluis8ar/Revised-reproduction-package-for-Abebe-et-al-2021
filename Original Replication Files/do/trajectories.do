...
do "utilities/_DefineQuadraticConstraintProgram"
do "utilities/_DefineQuadraticConstraintProgram_monthly"
...
drop mean_`outcome'
	...
	foreach g in 1 2  ///
			...
			local savename1 = "figures/figure`g'`tab'.png"  
...
drop mean_`outcome'
	...
	foreach g in 1    ///
 	...
 	foreach g in 1   ///
			...
			local savename1 = "figures/figure_a`tab'_q.png"  
			graph export "`savename1'", replace width(1000)
			...
			twoway	(rcap cit_`g' cib_`g' treat_w if treat_w>-8, xmtick(-7/14) xlabel(-7/14) xtitle(Fortnight)) ///
					(scatter  te_`g' treat_w,  $bounding ///
								...
								legend(off)) 	(line  flat treat_w if treat_w>-8, color(gs5))  ,   yline(0, lcolor(gs5) ) 	///
								ytitle("Effect on `outcomelabel'") 
			local savename1 = "figures/figure_a`tab'.png"  
			graph export "`savename1'", replace width(1000)
